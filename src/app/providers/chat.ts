...
export class Chat implements OnInit, OnDestroy {
        ...
        async (res) => {
          if (res.retcode == 0) {
            ...
            ChatItem = {
              ...
              message: MessageItem,
              _id:
                MESSAGE_LIST_PRE + group.group_id.split("@")[0].toLowerCase(),
              time: MessageItem.time,
              unix_time: new Date(MessageItem.time).getTime(),
              pic_url: "77ea4c86-b213-11ea-94f2-0242f326aa85.jpeg",
              count: 0,
            ...
            if (member_index != -1) {
              ChatItem.message.member.member_nick =
                GroupItem.members[member_index].member_nick;
              ChatItem.message.member.member_avatar =
                GroupItem.members[member_index].member_avatar;
            }
  ...
  async chatListUpdate(ChatItem: ChatItem) {
    ...
    list = _.forEach(list, function(item) {
      ...
      // 自己发出的私聊 或者 群聊
      if (
        ChatItem.message.from === that.dataService.userinfo.chat_jid ||
        ChatItem.message.member.member_no === that.dataService.userinfo.chat_jid
      ) {
        needAdd = false;
  ...
  onSaveMessage() {
    this.onSaveSub = this.getChatState().subscribe((res) => {
      if (res === "online") {
        this.newMessageSub = this.getNewMessage().subscribe(
          async (ChatItem: ChatItem) => {
            if (ChatItem) {
              if (
                this.lastMessage &&
                this.lastMessage.message.id === ChatItem.message.id
              ) {
                return true;
              }
  ...
  receiveMessageProcess() {
    this.receiveNewMessage
      .pipe(
        concatMap(async (MessageItem: MessageItem) => {
          if (
            MessageItem.member.member_no ===
              this.dataService.userinfo.chat_jid ||
            MessageItem.from === this.dataService.userinfo.chat_jid
          ) {
      ...
      this.getGroupPresenceStatus().subscribe(async (_) => {
        if (_) {
          ...
          if (!lastOfflineTime) {
            ...
            lastOfflineTime = new Date(
              new Date(lastOfflineTime).getTime() - 60 * 10 * 1000
            ).toISOString();
          }
          ...
          if (lastOfflineTime) {
            ...
            await this.queryChatHistoryByDate(
              this.messageQueryId,
              lastOfflineTime
            );
          }
  ...
  async sendPresenceToXmpp(group: GroupItem) {
    ...
    let userBareJid = this.dataService.userinfo.openfire_no
      .split("@")[0]
      .toLowerCase();
    const message = xml(
      "presence",
